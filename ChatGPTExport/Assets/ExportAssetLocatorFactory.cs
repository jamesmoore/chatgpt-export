...
namespace ChatGPTExport.Assets
{
    ...
    {
        public IAssetLocator GetAssetLocator(IEnumerable<ConversationAssets> parentDirectories, IDirectoryInfo destination)
        {
            var existingAssetLocator = new ExistingAssetLocator(destination);
            var assetLocators = parentDirectories.Select(p => new AssetLocator(p, destination, existingAssetLocator) as IAssetLocator).ToList();
